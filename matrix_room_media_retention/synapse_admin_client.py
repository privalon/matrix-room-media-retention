...
class SynapseAdminClient:
    ...
    def force_join_room(self, room_id: str) -> None:
        """`POST /_synapse/admin/v1/join/<room_id>` -- joins the given
        room as this admin account without needing an invite (confirmed
        against Synapse's own admin API docs). Idempotent: joining an
        already-joined room is a no-op, not an error."""
        url = f"{self._base_url}/_synapse/admin/v1/join/{room_id}"
        response = requests.post(url, headers=self._headers(), json={}, timeout=self._timeout_seconds)
